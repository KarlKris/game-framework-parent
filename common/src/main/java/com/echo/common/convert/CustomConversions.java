...
import com.echo.common.convert.core.GenericTypeResolver;
import com.echo.common.util.ObjectUtils;
import com.echo.common.util.Predicates;
import com.echo.common.util.SimpleTypeHolder;
import com.echo.common.util.Streamable;
...
public class CustomConversions {
    ...
    static {
        ...
        List<Object> defaults = new ArrayList<>(Jsr310Converters.getConvertersToRegister());
    ...
    private final Function<GenericConverter.ConvertiblePair, Class<?>> getReadTarget = convertiblePair -> getCustomTarget(
            convertiblePair.getSourceType(), convertiblePair.getTargetType(), readingPairs);

    private final Function<GenericConverter.ConvertiblePair, Class<?>> getWriteTarget = convertiblePair -> getCustomTarget(
            convertiblePair.getSourceType(), convertiblePair.getTargetType(), writingPairs);

    private final Function<GenericConverter.ConvertiblePair, Class<?>> getRawWriteTarget = convertiblePair -> getCustomTarget(
    ...
    /**
     * Populates the given {@link com.echo.common.convert.core.GenericConversionService} with the converters registered.
     *
     * @param conversionService {@link com.echo.common.convert.core.ConverterRegistry} to populate; must not be {@literal null}.
    ...
    /**
     * Registers the given converter in the given {@link com.echo.common.convert.core.GenericConversionService}.
     *
    ...
    /**
     * Registers the given {@link com.echo.common.convert.converter.GenericConverter.ConvertiblePair} as reading or writing pair depending on the type sides being basic
     * types.
     *
    ...
    /**
     * Inspects the given {@link com.echo.common.convert.converter.GenericConverter.ConvertiblePair ConvertiblePairs} for ones that have a source compatible type as source.
     ...
     */
    private Class<?> getCustomTarget(Class<?> sourceType, Class<?> targetType,
                                     Collection<GenericConverter.ConvertiblePair> pairs) {
    ...
    private static boolean requestedTargetTypeIsAssignable(Class<?> requestedTargetType,
                                                           Class<?> targetType) {
    ...
    static class ConversionTargetsCache {
        ...
        public Class<?> computeIfAbsent(Class<?> sourceType, Class<?> targetType,
                                        Function<GenericConverter.ConvertiblePair, Class<?>> mappingFunction) {
    ...
    static class TargetTypes {
        ...
        public Class<?> computeIfAbsent(Class<?> targetType, Function<GenericConverter.ConvertiblePair, Class<?>> mappingFunction) {
            ...
            if (optionalTarget == null) {
                optionalTarget = mappingFunction.apply(new GenericConverter.ConvertiblePair(sourceType, targetType));
    ...
    private static class ConverterRegistration {
        ...
        private ConverterRegistration(Object converter, GenericConverter.ConvertiblePair convertiblePair, StoreConversions storeConversions,
    ...
    public static class StoreConversions {
        ...
        private static boolean isAnnotatedWith(Class<?> type, Class<? extends Annotation> annotationType) {
            return AnnotationUtil.getAnnotation(type, annotationType) != null;
        }
        ...
        private ConverterRegistration register(Object converter, Class<?> source, Class<?> target, boolean isReading,
                                               boolean isWriting) {
            return register(converter, new GenericConverter.ConvertiblePair(source, target), isReading, isWriting);
        }

        private ConverterRegistration register(Object converter, GenericConverter.ConvertiblePair pair, boolean isReading,
                                               boolean isWriting) {
    ...
    protected static class ConverterConfiguration {
         ...
         * well as a {@link Collection} of {@link com.echo.common.convert.converter.GenericConverter.ConvertiblePair} for which to skip the registration of default converters.
         * <br />
